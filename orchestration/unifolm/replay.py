...
DEFAULT_EPISODE_DIR    = "/data/Unifolm/G1_WBT_Brainco_Collect_Plates_Into_Dishwasher/episode_000_data.json"
...
# ── Dex3 hand mapping ─────────────────────────────────────────────────────────
# BrainCo hand_cmd is trigger * 0.8 (collection pipeline caps commanded close at
# 0.8 on the 0-1 scale), replicated across all finger dims of a hand.
# closure = hand_cmd / 0.8 recovers the operator's trigger travel in [0, 1].
_BRAINCO_CMD_MAX = 0.8

# Close preset from G1GripperInverseKinematicsSolver._get_middle_close_q_desired —
# the same pose the teleop trigger path sends to Dex3. Left hand; right = negation.
_DEX3_CLOSE_LEFT = np.array([0.0, 0.7, 0.7, -1.0, -1.5, -1.0, -1.5], dtype=np.float32)

# BrainCo per-hand layout: [thumb open/close, thumb lateral, index, middle, ring,
# little]. Thumb lateral (idx 1) is pinned at 0.8 and carries no closure signal.
_BRAINCO_FINGER_DIMS = [0, 2, 3, 4, 5]


def extract_hand_closure(frame, source="action"):
    """Return (left_closure, right_closure) in [0, 1], or None if no hand data.

    Reads Unifolm hand_cmd (source="action") or hand_state (source="state"),
    12 = left(6) + right(6) in BrainCo layout.
    """
    key = "action" if source == "action" else "observation"
    entry = frame.get(key)
    if entry is None:
        return None
    hand12 = entry.get("hand_cmd" if key == "action" else "hand_state")
    if hand12 is None:
        return None
    hand12 = np.asarray(hand12, dtype=np.float32)
    if hand12.shape[0] != 12:
        return None
    left = hand12[:6][_BRAINCO_FINGER_DIMS].max()
    right = hand12[6:][_BRAINCO_FINGER_DIMS].max()
    return (
        float(np.clip(left / _BRAINCO_CMD_MAX, 0.0, 1.0)),
        float(np.clip(right / _BRAINCO_CMD_MAX, 0.0, 1.0)),
    )


def closure_to_dex3(left_closure, right_closure):
    """Map per-hand closure [0, 1] to Dex3 7-DOF joint targets (open = zeros)."""
    q_left = left_closure * _DEX3_CLOSE_LEFT
    q_right = right_closure * (-_DEX3_CLOSE_LEFT)
...
def extract_qpos_quat(frame, source="action"):
    ...
    Supports two formats:
      - old:     frame["states"/"actions"]["qpos"] (29) + ["quat"] (4, relative)
      - Unifolm: frame["observation"/"action"]["robot_q_current"/"robot_q_desired"]
                 (36 = root pos xyz [0:3] + root quat wxyz [3:7] + 29 joints [7:36]).
                 hand_state / hand_cmd are ignored.

    `source` selects "action" (target values) or "state" (measured values).
    """
    if "observation" in frame or "action" in frame:
        key = "action" if source == "action" else "observation"
        entry = frame.get(key)
        if entry is None:
            return None
        q36 = entry.get("robot_q_desired" if key == "action" else "robot_q_current")
        if q36 is None:
            return None
        q36 = np.asarray(q36, dtype=np.float32)
        if q36.shape[0] != 36:
            return None
        return q36[7:36], q36[3:7]
    ...
    # old format stores quat only in states (relative to recording start)
    quat_src = states if states is not None else entry
...
def replay(episode_dir, zmq_host, zmq_pub_port, zmq_topic,
    ...
    try:
        for i, frame in enumerate(frames):
            ...
            if send_hands:
                closure = extract_hand_closure(frame, source=source)
                if closure is not None:
                    left_hand, right_hand = closure_to_dex3(*closure)
...
def main():
    ...
    parser.add_argument(
        "--source", type=str, choices=["action", "state"], default="action",
        help="Send action targets (robot_q_desired / actions.qpos) or measured state (default: action)",
    )
    parser.add_argument(
        "--no-hands", action="store_true",
        help="Do not send Dex3 hand joints derived from BrainCo hand_cmd",
    )
    parser.add_argument(
