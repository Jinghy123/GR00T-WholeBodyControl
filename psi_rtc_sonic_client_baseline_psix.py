...
# TASK_INSTRUCTION = "gather up the yellow shirt and turn right and put them into the laundry basket"
...
# TASK_INSTRUCTION = "grasp the transparent bottle, open the top drawer of the kitchen island, place the bottle inside, and close the drawer"
...
#
TRAIN_IMAGE_SIZE = (672, 384)   # (W, H) the pack declares, i.e. what the server resizes


def to_train_geometry(frame):
    """Resize a live camera frame to the training pack's image geometry (672x384)."""
    if frame is None or (frame.shape[1], frame.shape[0]) == TRAIN_IMAGE_SIZE:
        return frame
    return cv2.resize(frame, TRAIN_IMAGE_SIZE, interpolation=cv2.INTER_AREA)


# The old black-band padding, kept for reference in case a pack with the band comes back:
...
#   default:        states(43) = leg(12) + waist(3) + arm(14) + hand(14)
#                                [was documented as hand(14) + arm(14) + leg(15)]
...
#
# The state order is the one meta/info.json names for g1_sonic_lerobot_0810_merged_train,
# which is what the served checkpoint's field.state_min/max are indexed by:
#   0:12  left/right leg   (hip pitch,roll,yaw, knee, ankle pitch,roll) x2
#   12:15 waist yaw, roll, pitch
#   15:29 left/right arm   (shoulder pitch,roll,yaw, elbow, wrist roll,pitch,yaw) x2
#   29:43 left/right hand  (thumb 0,1,2, middle 0,1, index 0,1) x2
#   43:45 neck yaw, pitch
# i.e. exactly `body_q_measured` (29) ++ left_hand_q (7) ++ right_hand_q (7) ++ neck (2),
# no permutation. The POST-TRAIN packs (psix_sonic_v1_*) hold the same 45 joints as
# hand|arm|leg|waist|neck instead -- see psi's scripts/data/reorder_state_columns.py --
# so a checkpoint finetuned on a reordered pack would need the other order here.
HAND_DIM = 14
NECK_DIM = 2
TOKEN_DIM = 64
    ...
    def _send_thread(self):
        ...
        while self._running and running.is_set():
            try:
                ...
                leg_states = body_q[:15]    # leg(12) + waist(3)
                ...
                if self._recorder is not None:
                    ...
                    # It is the frame in TRAINING geometry (was: the PADDED frame), so the
                    # recording is pixel-for-pixel what the policy saw; the recorder
                    # derives info.json's shape from it.
                    with self._obs_lock:
                        self._latest_obs = (
                            frame.copy(),
