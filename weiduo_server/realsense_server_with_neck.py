"""
Native RealSense head-camera server + 2-DOF neck motor (robot-side).

This merges the two existing servers:
  - Camera side is a verbatim port of realsense_native_server.py (the WORKING
    native pyrealsense2 path, `ruohai` env) — no ZED, no pyzed import.
  - Neck side is a verbatim port of the NeckMotor class from
    realsense_server.py (Dynamixel IDs 0=yaw, 1=pitch on /dev/ttyUSB0, driven
    by the SMPL-X-derived `[yaw, pitch]` ZMQ stream from pose_publisher.py).

Consumers served simultaneously (recording / viewer / Pico never steal frames
from each other):

  1. ZMQ REP  (default tcp://0.0.0.0:5558) — recording / inference client
     (RSCamera / RealSenseClient). 3-part multipart reply, identical to
     realsense_native_server.py so desktop clients stay unchanged:
       Part 0 — RGB   JPEG   (640x480 BGR)
       Part 1 — IR    JPEG   (left|right hstacked; b"" if --no-ir)
       Part 2 — Depth raw    (z16 uint16 LE; b"" if --no-depth)
  2. ZMQ PUB  (default tcp://0.0.0.0:5559) — live viewer stream, same 3 parts.
  3. Pico H.264 (--enable-pico) — GStreamer x264 over TCP to the Pico headset.
  4. ZMQ PUB  (default tcp://*:5560) — neck present-position [yaw_rad,
     pitch_rad] at the neck control rate, for the data recorder. Only active
     with --enable-neck-motor; disable with --neck-state-pub ''.

Usage (on the robot, `ruohai` env — see weiduo_server/usage.md for the full
checklist):
...
Environment overrides: RS_ZMQ_BIND, RS_VIEWER_PUB, RS_FPS, RS_WIDTH, RS_HEIGHT,
                       ...
                       POSE_ZMQ, NECK_MOTOR, NECK_STATE_PUB
"""
...
# Neck-motor configuration — kept identical to realsense_server.py; edit these
...
# ──────────────────────────────────────────────────────────────────────────────
# Pico Video Streamer (GStreamer H.264 + TCP) — streams RGB or IR L|R
# ──────────────────────────────────────────────────────────────────────────────


class PicoVideoStreamer:
    """Streams the native RGB BGR frame (or the IR L|R pair) to the Pico
    headset via GStreamer x264, framed as [4-byte BE length][NALUs] over TCP.

    3 worker threads:
      _push_loop        — waits for fresh frames, feeds appsrc
...
# ──────────────────────────────────────────────────────────────────────────────
# Neck motor driver (pose_publisher.py head pose → 2x Dynamixel)
# ──────────────────────────────────────────────────────────────────────────────
...
def start_server(cfg: argparse.Namespace) -> None:
    ...
    if cfg.enable_neck_motor:
        ...
        if not neck_motor.start():
            print(
                "\033[91m[Neck] Failed to start; "
                "continuing without motor control.\033[0m"
            )
            neck_motor = None
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Native RealSense head-camera server + 2-DOF neck motor. "
                    "REP recording + PUB viewer + optional Pico H.264 + "
                    "optional Dynamixel neck driven by pose_publisher.py."
    )
    ...
    p.add_argument(
        ...
        help=(
            "Drive the 2-DOF neck (Dynamixel IDs 0=yaw, 1=pitch on "
            "/dev/ttyUSB0) from SMPL-X-derived neck angles published by "
            "pose_publisher.py."
        ),
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        help=(
            ...
            "[yaw_rad, pitch_rad]. Read from the Dynamixels at the neck "
            "control rate so a recorder can log it into data.json. "
            "Set to empty string to disable. Default 'tcp://*:5560'."
        ),
    )
...
def main() -> None:
    ...
    if ns.enable_neck_motor and not ns.pose_zmq:
        raise SystemExit(
            "--enable-neck-motor requires --pose-zmq tcp://<desktop-ip>:5570 "
            "(pose_publisher.py must be running on the desktop)"
        )
    start_server(ns)
