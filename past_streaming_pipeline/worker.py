...
import threading
import time
from multiprocessing import Event, Manager, Process, Queue, shared_memory
...
class RobotDataWorker:
    ...
    def _recv_zmq_frame(self):
        ...
        rgb_array = np.frombuffer(parts[0], np.uint8) if parts[0] else None
        stereo_array = np.frombuffer(parts[1], np.uint8) if len(parts) > 1 and parts[1] else None
        lw_rgb_array = np.frombuffer(parts[2], np.uint8) if len(parts) > 2 and parts[2] else None
        rw_rgb_array = np.frombuffer(parts[3], np.uint8) if len(parts) > 3 and parts[3] else None

        return rgb_array, stereo_array, lw_rgb_array, rw_rgb_array
    ...
    def get_robot_data(self, time_curr):
        ...
        robot_data = {
            ...
            "left_wrist_image": f"color_left_wrist/frame_{self.frame_idx:06d}.jpg",
            "right_wrist_image": f"color_right_wrist/frame_{self.frame_idx:06d}.jpg",
        }
    ...
    def _write_image_data(self, rgb_array, stereo_array=None, lw_rgb_array=None, rw_rgb_array=None):
        ...
        if rgb_array is not None:
            ...
            logger.debug(f"Saved color frame to {color_filename}")
        else:
            logger.error(f"failed to save image {self.frame_idx}")

        if stereo_array is not None:
            stereo_filename = os.path.join(
                self.shared_data["dirname"], f"color_stereo/frame_{self.frame_idx:06d}.jpg"
            )
            self.async_image_writer.write_image(stereo_filename, stereo_array)

        if lw_rgb_array is not None:
            lw_filename = os.path.join(
                self.shared_data["dirname"], f"color_left_wrist/frame_{self.frame_idx:06d}.jpg"
            )
            self.async_image_writer.write_image(lw_filename, lw_rgb_array)

        if rw_rgb_array is not None:
            rw_filename = os.path.join(
                self.shared_data["dirname"], f"color_right_wrist/frame_{self.frame_idx:06d}.jpg"
            )
            self.async_image_writer.write_image(rw_filename, rw_rgb_array)

    def _write_robot_data(self, rgb_array, stereo_array=None, lw_rgb_array=None, rw_rgb_array=None, reuse=False):
        logger.debug(f"Worker: writing robot data")
        self._write_image_data(rgb_array, stereo_array, lw_rgb_array, rw_rgb_array)
    ...
    def process_data(self):
        ...
        rgb_array, stereo_array, lw_rgb_array, rw_rgb_array = self._recv_zmq_frame()
        logger.debug("got frame")
        if stereo_array is not None:
            self._send_image_to_teleoperator(stereo_array)
        elif rgb_array is not None:
            self._send_image_to_teleoperator(rgb_array)
        time_curr = time.time()
        ...
        if self.is_first:
            ...
            self._write_robot_data(rgb_array, stereo_array, lw_rgb_array, rw_rgb_array)
        ...
        if next_capture_time - time_curr >= 0:
            ...
            if self.last_robot_data is not None:
                self._write_robot_data(rgb_array, stereo_array, lw_rgb_array, rw_rgb_array, reuse=True)
            else:
                logger.error("worker: no previous data available, generating null data")
                self._write_robot_data(None, None, None, None, reuse=True)
