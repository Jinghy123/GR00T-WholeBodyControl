...
DEFAULT_EPISODE_DIR    = "/home/xiawei/data/log_debug/bad"
...
# Mapping from data ordering to standard G1 / mujoco ordering.
# data:  0:l_hip_pitch 1:r_hip_pitch 2:waist_yaw 3:l_hip_roll 4:r_hip_roll 5:waist_roll
#        6:l_hip_yaw 7:r_hip_yaw 8:waist_pitch 9:l_knee 10:r_knee
...
# mujoco/G1: 0:l_hip_pitch 1:l_hip_roll 2:l_hip_yaw 3:l_knee 4:l_ankle_pitch 5:l_ankle_roll
...
_DATA_TO_MUJOCO_DOF = np.array(
    [0, 3, 6, 9, 13, 17, 1, 4, 7, 10, 14, 18, 2, 5, 8, 11, 15, 19, 21, 23, 25, 27, 12, 16, 20, 22, 24, 26, 28],
    dtype=np.int32,
)

def _data29_to_mujoco29(joint_pos29: np.ndarray) -> np.ndarray:
    jp = np.asarray(joint_pos29, dtype=np.float32).reshape(29)
    return jp[_DATA_TO_MUJOCO_DOF].astype(np.float32).copy()
...
def _data29_to_isaaclab29(joint_pos29: np.ndarray) -> np.ndarray:
    return _mujoco29_to_isaaclab29(_data29_to_mujoco29(joint_pos29))
...
def replay(episode_dir, zmq_host, zmq_pub_port, zmq_topic,
    ...
    try:
        for i, frame in enumerate(frames):
            ...
            qpos = np.array(states["joint_pos"], dtype=np.float32)   # (29,)
            actions = np.array(states["q_target"], dtype=np.float32)  # (29,)
            q_rel = np.array([1.0, 0.0, 0.0, 0.0], dtype=np.float32)  # (4,) relative to recording start

            # qpos_isaac = _data29_to_isaaclab29(qpos)
            qpos_isaac = _data29_to_isaaclab29(actions)
