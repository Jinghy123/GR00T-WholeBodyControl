"""VENDORED copy of psi/src/psi/deploy/wire_contracts.py for the robot client.

The robot env has no psi checkout, so the cross-socket byte contracts live here
as a verbatim copy of the helper bodies. Cross-repo consistency is enforced by
the frozen golden vectors (test_hlpwm_wire_vendored.py here, tests/
test_wire_contracts.py in psi) — the SAME digests are asserted against both
implementations, so silent drift fails a test on whichever side changed.

Rules for editing:
  * never edit a function body here without making the identical edit in psi's
    wire_contracts.py, bumping the affected *_VERSION, and regenerating goldens;
  * additions that don't affect byte layout still belong in psi first.
