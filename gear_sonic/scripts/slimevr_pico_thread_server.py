"""
slimevr_pico_thread_server.py

SlimeVR full-body tracking + Pico controller triggers (gripper IK) → GMR IK →
Unitree G1 joint states, published to the sonic deploy over ZMQ (v1 protocol),
plus a 2-DOF neck stream.

This is the trigger-hand counterpart to slimevr_manus_thread_server.py. The body
source is SlimeVR (VMC/OSC); the ONLY difference is the hand path: instead of the
Manus gloves + dex-retargeting, the hands are driven by the Pico controller
triggers through the legacy gripper-IK solver (the same `use_pico_hand` path used
by pico_manus_thread_server.py). Everything downstream (GMR retarget, v1 ZMQ
contract, neck ZMQ) matches the validated GR00T pipeline so g1_data_server.py /
realsense_server.py / the WBC deploy consume it unchanged.

Trigger → hand: the right/left trigger continuously closes the matching gripper
(0=open, 1=closed); see generate_finger_data / G1GripperInverseKinematicsSolver.
...
Pipeline:
  SlimeVR app → VMC/OSC (UDP :39539)
       ...
       → reorder to IsaacLab → ZMQ v1 (joint_pos/joint_vel/body_quat/frame_index
         + left/right_hand_joints from Pico-trigger gripper IK)   on :5556
...
Hand note: the Pico controllers stream through the XRoboToolkit PC service. Even
though the body comes from SlimeVR (not the Pico headset), the controllers must
be connected and the local service running so xrt.get_*_trigger() returns data.
This script can start the service itself (START_PICO_SERVICE) or attach to one
...
Configuration: edit the CONFIG block below and run with no arguments:
    python gear_sonic/scripts/slimevr_pico_thread_server.py
"""
...
# --- Pico 扳机手（trigger -> 夹爪开合，走 gripper IK，连续）---
ENABLE_HAND         = True            # False=只发身体+脖子
PINCH               = False           # 默认 no-pinch（中指普通抓握）；True=拇指+食指 pinch（或命令行 --pinch）
HALF_THUMB          = False           # True=拇指最多张开一半（闭合下限 0.5）；食指/中指不受限（或命令行 --half）
START_PICO_SERVICE  = True            # True=本脚本启动 XRoboToolkit 本地 service；
                                      # 若已有别的进程（如 pico_manus）起了 service，设 False
PICO_SERVICE_SH     = "/opt/apps/roboticsservice/runService.sh"
...
# ── XRoboToolkit SDK (Pico controllers) ───────────────────────────────────────
try:
    import xrobotoolkit_sdk as xrt
except ImportError:
    xrt = None

# ── Pico-trigger Dex3 hands — reuse the validated use_pico_hand path from
#    pico_manus_thread_server (gripper IK driven by the controller triggers) ────
try:
    ...
    _PICO_OK = True
except Exception as _pico_err:  # pragma: no cover - env dependent
    print(f"Warning: Pico hand reuse unavailable ({_pico_err}); running body+neck only.")
    compute_hand_joints_from_inputs = get_controller_inputs = init_hand_ik_solvers = set_pinch = set_half_thumb = None
    _PICO_OK = False
...
class SlimevrPicoGMRServer:
    def __init__(self):
        ...
        # ── Pico-trigger Dex3 hands (gripper IK) ──────────────────────────────
        self.left_hand_solver = None
        self.right_hand_solver = None
        self._xrt_ok = False
        if ENABLE_HAND and _PICO_OK:
            if xrt is None:
                print("[pico] xrobotoolkit_sdk not importable; hands disabled (body+neck only).")
            else:
                if START_PICO_SERVICE:
                    try:
                        subprocess.Popen(["bash", PICO_SERVICE_SH])
                        print(f"[pico] started XRoboToolkit service: {PICO_SERVICE_SH}")
                    except Exception as e:
                        print(f"[pico] could not start service ({e}); assuming one is already running.")
                try:
                    xrt.init()
                    self._xrt_ok = True
                    print("[pico] xrt.init() OK — reading controller triggers.")
                except Exception as e:
                    print(f"[pico] xrt.init() failed ({e}); hands disabled (body+neck only).")
                if self._xrt_ok:
                    set_pinch(PINCH)
                    set_half_thumb(HALF_THUMB)
                    print(f"[pico] hand gesture: {'PINCH (thumb+index)' if PINCH else 'GRASP (middle)'}"
                          f", thumb={'HALF-open' if HALF_THUMB else 'full'}")
                    self.left_hand_solver, self.right_hand_solver = init_hand_ik_solvers()
        elif ENABLE_HAND and not _PICO_OK:
            print("[pico] pico hand path unavailable; running body+neck only.")
    ...
    def _hands(self):
        """(left[7], right[7]) Dex3 joints from the Pico triggers, zeros if no SDK."""
        if not self._xrt_ok or self.left_hand_solver is None:
        ...
        try:
            ...
            z = np.zeros(7, dtype=np.float32)
            return z, z.copy()
        # Same arg order as pico_manus_thread_server: (lt, lg, rt, rg).
        lh, rh = compute_hand_joints_from_inputs(
            self.left_hand_solver, self.right_hand_solver, lt, lg, rt, rg
        )
...
def main():
    global PINCH, HALF_THUMB
    import argparse

    p = argparse.ArgumentParser(
        description="SlimeVR body + Pico-trigger hands → GMR → G1 (ZMQ v1)."
    )
    # Pinch (thumb+index) vs grasp (middle), and half-open thumb. Default to the
    # CONFIG values so the constants stay the defaults and the flags just override.
    p.set_defaults(pinch=PINCH, half_thumb=HALF_THUMB)
    p.add_argument(
        "--pinch", dest="pinch", action="store_true",
        help="Pico trigger does a thumb+index PINCH",
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        help="Limit the thumb to half-open (closure floored at 0.5); index/middle keep full range",
    )
    args = p.parse_args()
    PINCH = args.pinch
    HALF_THUMB = args.half_thumb

    SlimevrPicoGMRServer().run()
